...
import { optimizePDF, getPDFPageCount, downloadPDF } from '../utils/pdfHelpers';
import ToolHeader from './ToolHeader';

export default function OptimizeTool({ onBack }: { onBack: () => void }) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // IMPORTANT: allows selecting same file again
    e.target.value = '';
  };
  ...
  const handleOptimizePDF = async () => {
    ...
    try {
      ...
      downloadPDF(result, `optimized_${file.name}`);
    } catch (error) {
      console.error('Error optimizing PDF:', error);
      alert('Error optimizing PDF. Please try again.');
    } finally {
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                  <strong>Optimization:</strong> The PDF will be compressed and internally
                  optimized to reduce size while keeping readability.
                </p>
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  {isProcessing ? 'Processing...' : 'Optimize PDF'}
                </button>
              </div>
